 ...
 *
 * Kernkonzept: Der Kundentyp bestimmt, welche Identitäts-Felder gezeigt werden:
 *   - Geschäftlich → "Firmenname"
 *   - Privat       → "Anrede" + "Titel" + "Vorname" + "Nachname"
 *
 * Der Aufrufer entscheidet via `variant`:
 *   - "full"    : KingBill-Editor-Layout ("Kunden bearbeiten"): zweispaltiger
 *                 Kopfbereich (links Identität/Gruppe/UID, rechts Adresse +
 *                 Kontakt) und darunter eine Tab-Leiste (Kommentar |
 *                 Rechnungsadresse | Zahlungsbedingungen | Wichtige Daten).
 *                 Es werden NUR die schon vorhandenen Datenfelder gerendert.
 *   - "minimal" : Nur Pflicht-/Kernfelder (für Inline-Erstellung), klassisches
 *                 Stack-Layout mit Kundentyp-Toggle oben.
 *
...
export function CustomerForm({
  ...
  // ─────────────────────────────────────────────────────────────────
  // variant="minimal" — kompaktes Stack-Layout (Inline-Erstellung in
  // CustomerSelect / CreateProjectDialog). Unverändert gegenüber vorher.
  // ─────────────────────────────────────────────────────────────────
  if (variant === "minimal") {
    return (
  ...
  // ─────────────────────────────────────────────────────────────────
  // variant="full" — KingBill-Editor-Layout „Kunden bearbeiten":
  // zweispaltiger Kopfbereich + Tab-Leiste unten. Gleiche Felder wie
  // zuvor, nur KingBill-konform angeordnet (keine neuen Datenfelder).
  // ─────────────────────────────────────────────────────────────────
  return (
    <div className="space-y-4">
      {/* Kopfbereich: links Identität, rechts Adresse + Kontakt */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
        {/* ── Linke Spalte: Kundennummer / Gruppe / Identität ── */}
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              {editId ? (
                ...
                <>
                  <Input value="" disabled placeholder="Wird automatisch vergeben" />
                  <p className="text-[10px] text-muted-foreground mt-0.5">
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v) => setKundentyp(v as "geschaeftskunde" | "privatkunde")}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="geschaeftskunde">
                    <span className="flex items-center gap-1.5"><Building className="w-3.5 h-3.5" /> Geschäftskunde</span>
                  </SelectItem>
                  <SelectItem value="privatkunde">
                    <span className="flex items-center gap-1.5"><User className="w-3.5 h-3.5" /> Privatkunde</span>
                  </SelectItem>
                </SelectContent>
              </Select>
            </div>
      ...
      <div className="border-t border-border pt-3">
        <div className="flex flex-wrap gap-1.5" role="tablist" aria-label="Weitere Kundendaten">
          {KB_EDITOR_TABS.map((t) => (
            <button
              ...
              type="button"
              role="tab"
              aria-selected={kbTab === t.val}
              onClick={() => setKbTab(t.val)}
              className={kbTab === t.val ? "kb-tab-active" : "kb-tab"}
            >
              {t.label}
            </button>
          ))}
        ...
        <div className="mt-2 rounded-md border border-border bg-muted/20 p-3 min-h-[180px]">
          {kbTab === "kommentar" && (
            <div>
