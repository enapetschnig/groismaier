        ...
        <div className="p-4 sm:p-5 space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            {form.art !== "maschine" && (
              <>
                <div>
                  <label className="block text-xs font-semibold mb-1" htmlFor="fz-marke">Marke / Modell</label>
                  <input
                    id="fz-marke"
                    className="kb-input w-full"
                    value={form.marke_modell}
                    onChange={(e) => setForm(f => ({ ...f, marke_modell: e.target.value }))}
                    placeholder="z.B. VW Crafter 35"
                  />
                </div>
                <div>
                  <label className="block text-xs font-semibold mb-1" htmlFor="fz-fin" title="Feld E am Zulassungsschein">Fahrgestellnummer (FIN)</label>
                  <input
                    id="fz-fin"
                    className="kb-input w-full font-mono"
                    value={form.fahrgestellnummer}
                    onChange={(e) => setForm(f => ({ ...f, fahrgestellnummer: e.target.value.toUpperCase() }))}
                    placeholder="17-stellig"
                  />
                </div>
                <div>
                  <label className="block text-xs font-semibold mb-1" htmlFor="fz-ez">Erstzulassung</label>
                  <input
                    id="fz-ez"
                    type="date"
                    className="kb-input w-full"
                    value={form.erstzulassung}
                    onChange={(e) => setForm(f => ({ ...f, erstzulassung: e.target.value }))}
                  />
                </div>
                {editId && vehicles.find(v => v.id === editId)?.zulassungsschein_path && (
                  <div className="flex items-end">
                    <button
                      type="button"
                      className="kb-btn flex items-center gap-1.5 text-sm"
                      onClick={async () => {
                        const pfad = vehicles.find(v => v.id === editId)?.zulassungsschein_path;
                        if (!pfad) return;
                        const { data, error } = await supabase.storage
                          .from("vehicle-documents")
                          .createSignedUrl(pfad, 3600);
                    ...
                    >
                      <FileText className="h-4 w-4 text-kb-blue" /> Zulassungsschein öffnen
                    </button>
                  </div>
                )}
              </>
